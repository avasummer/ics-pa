...
#define ECALL(dnpc) { bool success; dnpc = isa_raise_intr(isa_reg_str2val("a7", &success), s->pc); assert(success); }
...
__VA_ARGS__ ; \
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = (uint64_t)src1 < (uint64_t)imm);
