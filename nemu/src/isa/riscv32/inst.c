...
#define immB() do { *imm = (SEXT( (BITS(i, 31, 31) << 12 | BITS(i, 31, 25) << 5 | BITS(i, 11, 8) << 1 | BITS(i, 7, 7) << 11), 12)); } while(0)
...
__VA_ARGS__ ; \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(rd) = (int32_t)(imm + src1));
  ...
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, if(!src2)R(rd) = UINT64_MAX;R(rd) = (uint64_t)src1 / (uint64_t)src2);
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, if(!src2)R(rd)=-1;else if(src1==INT32_MIN && src2==-1)R(rd)=src1;else R(rd) = (int32_t)src1 / (int32_t)src2);
  INSTPAT("0000001 ????? ????? 101 ????? 01110 11", divuw  , R, if(!src2)R(rd)=-1;else if(src1==INT32_MIN && src2==-1)R(rd)=src1;else R(rd) = (int32_t)((uint32_t)src1 / (uint32_t)src2));
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, Log("%ld %ld %d\n",src1,src2,(int64_t)src1>=(int64_t)src2);if((int64_t)src1 >= (int64_t)src2){Log(" go 0x%lx+0x%lx = 0x%lx\n",s->pc,imm,(int64_t)s->pc+imm);s->dnpc = s->pc+imm;});
